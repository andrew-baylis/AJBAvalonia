...
public class RadioCheckEnumGroup : Panel
{
    ...
    public static readonly StyledProperty<Thickness?> ButtonMarginProperty =
        AvaloniaProperty.Register<RadioCheckEnumGroup, Thickness?>(nameof(ButtonMargin));

    public static readonly StyledProperty<int> ColumnsProperty =
        AvaloniaProperty.Register<RadioCheckEnumGroup, int>(nameof(Columns));

    public static readonly DirectProperty<RadioCheckEnumGroup, Type?> EnumTypeProperty =
        AvaloniaProperty.RegisterDirect<RadioCheckEnumGroup, Type?>(nameof(EnumType), o => o.EnumType,
            (o, v) => o.EnumType = v);

    public static readonly StyledProperty<int> FirstColumnProperty =
        AvaloniaProperty.Register<RadioCheckEnumGroup, int>(nameof(FirstColumn));

    public static readonly StyledProperty<GroupOrientation> OrientationProperty =
        AvaloniaProperty.Register<RadioCheckEnumGroup, GroupOrientation>(nameof(Orientation));

    public static readonly StyledProperty<int> RowsProperty =
        AvaloniaProperty.Register<RadioCheckEnumGroup, int>(nameof(Rows));
    ...
    {
        AffectsArrange<RadioCheckEnumGroup>(ButtonMarginProperty, OrientationProperty, FirstColumnProperty,
            ColumnsProperty, RowsProperty);
    ...
    public object? SelectedEnumValue
    {
        ...
        set
        {
            ...
            if (IsLoaded && !_inClickEvent)
            {
                ...
                RaisePropertyChanged(SelectedEnumValueProperty, null, _selectedEnumValue);
            }
        }
    }
    ...
    private void CbOnIsCheckedChanged(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            if (sender is CheckBox checkBox)
            {
                ...
                var singleValue = _lookupControls.Where(item => item.Key.IsChecked == true && !item.Value.multiple)
                    .Aggregate(0, (current, item) => current | item.Value.v);

                //now check if other multiples need to be set
                foreach (var lookupControl in _lookupControls)
                {
                    if (lookupControl.Key != checkBox && lookupControl.Value.multiple &&
                        (singleValue & lookupControl.Value.v) == lookupControl.Value.v)
                    {
    ...
    {
        if (EnumType != null && IsLoaded && !_inClickEvent && value?.GetType() == EnumType)
        {
